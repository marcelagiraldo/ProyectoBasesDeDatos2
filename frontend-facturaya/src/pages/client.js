  ...
  const handleSubmit = async (e)=>{
    e.preventDefault();
    let errMsg = ""
    if(!clientData.nombre || !clientData.numero_documento || !clientData.telefono || !clientData.email || !clientData.direccion || !clientData.departamento || !clientData.ciudad){
      errMsg = "Todos los campos son requeridos"
      setErrorMsg(errMsg)
    }

    if((errMsg.length===0) && clientData.numero_documento){
      await axios.patch(`http://localhost:3005/clientes/${clientData.numero_documento}`,clientData).then((res)=>{
      ...
      await axios.post('http://localhost:3005/clientes',clientData).then((res)=>{
        console.log("Datos recibidos:", res.data);
        setclients(res.data)
      }).catch((error)=>{
        console.error("Error al obtener los clientes:", error);
    ...
    <div className="App">
      ...
      <div className='table-box'>
        {isModalOpen && <div className='addeditpopup'>
          ...
          <button className='addClientBtn' onClick={handleSubmit}>{clientData.numero_documento?"Update Student":"Agregar"}</button>
          </div>
          <div className='derecho'><div className='popupdiv'>
